...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.chequing_account = ChequingAccount(123, 456, 1000.0, date(2024, 8, 1), 200.0, 0.03)
    ...
    def test_init_valid(self):
        ...
        self.assertEqual(1000.0, self.chequing_account._BankAccount__balance)
        self.assertEqual(date(2024, 8, 1), self.chequing_account._date_created)
        self.assertEqual(200.0, self.chequing_account._ChequingAccount__overdraft_limit)
        self.assertEqual(0.03, self.chequing_account._ChequingAccount__overdraft_rate)

    def test_overdraft_limit_invalid(self):
        chequing_account = ChequingAccount(123, 456, 1000.0, date(2024, 8, 1), "overdraft", 0.03)
    ...
    def test_date_created_invalid(self):
        chequing_account = ChequingAccount(123, 456, 1000.00, 8/1/2024, 200.0, 0.03)
    ...
    def test_get_service_charges_with_balance_less_than_overdraft_limit(self):
        chequing_account = ChequingAccount(123, 456, 100.0, date(2024, 8, 1), 200.0, 0.03)
        self.assertAlmostEqual(3.5,round(chequing_account.get_service_charges(),2))

    def test_get_service_charges_with_balance_equals_to_overdraft_limit(self):
        chequing_account = ChequingAccount(123, 456, 200.0, date(2024, 8, 1), 200.0, 0.03)
        self.assertAlmostEqual(chequing_account.BASE_SERVICE_CHARGE, round(chequing_account.get_service_charges(),2))

    def test_str(self):
        expected = 'Account Number:123 Balance:$1,000.00\nOverdraft Limit: $200.0 Overdraft Rate: 3.00% Account Type: Chequing'
